...
def convert_search_boolean_to_snuba_query(search_boolean):
    def convert_term(term):
        if isinstance(term, SearchFilter):
            ...
            return convert_aggregate_filter_to_snuba_query(term)
...
def convert_aggregate_filter_to_snuba_query(aggregate_filter):
...
def get_filter(query=None, params=None):
    ...
    for term in parsed_terms:
        if isinstance(term, SearchFilter):
            ...
            converted_filter = convert_aggregate_filter_to_snuba_query(term)
            if converted_filter:
